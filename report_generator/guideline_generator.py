    ...
    def _initialize_descriptions(self):
        ...
        return {
            ...
            'file_size_distribution': {
                'title': '파일 크기 분포',
                'description': '이 차트는 데이터셋 내 이미지들의 파일 크기 분포를 히스토그램 형태로 시각화합니다. X축은 파일 크기를 메가바이트(MB) 단위로 표시하고, Y축은 해당 크기 구간에 속하는 이미지의 개수를 나타냅니다. 각 막대는 특정 크기 구간의 이미지 수량을 의미하며, 이 차트를 통해 데이터셋의 저장 공간 사용 패턴을 파악할 수 있습니다. 저장 공간 최적화 전략 수립과 데이터 품질 평가에 중요한 정보를 제공합니다.',
            ...
            'resolution_distribution': {
                'title': '해상도 분포',
                'description': '이 차트는 데이터셋 내 이미지들의 해상도 분포를 막대 차트 형태로 시각화합니다. X축은 이미지의 해상도(가로 픽셀 × 세로 픽셀)를 표시하고, Y축은 해당 해상도를 가진 이미지의 개수를 나타냅니다. 각 막대는 특정 해상도를 가진 이미지의 수량을 의미하며, 이 차트를 통해 데이터셋의 해상도 다양성을 확인할 수 있습니다. 모델 학습 데이터의 품질과 전처리 필요성을 평가하는 데 중요한 정보를 제공합니다.',
                'category': 'image_attributes'
            },
            
            'noise_vs_sharpness': {
                'title': '노이즈 vs 엣지니스',
                'description': '이 차트는 이미지의 노이즈 레벨과 엣지니스라는 두 가지 품질 속성 간의 관계를 산점도 형태로 시각화합니다. X축은 노이즈 레벨을, Y축은 엣지니스 값을 나타내며, 각 점은 개별 이미지를 의미합니다. 이 차트를 통해 두 속성 간의 상관관계를 파악할 수 있으며, 데이터 전처리 시 노이즈 제거의 필요성과 이미지 품질 개선 포인트를 파악할 수 있습니다.',
                'category': 'image_attributes'
            },
            ...
            'embeddings_pca': {
                'title': 'PCA 2D 임베딩',
                'description': '이 차트는 고차원 이미지 임베딩 데이터를 주성분 분석(PCA)을 통해 2차원으로 축소한 결과를 산점도 형태로 시각화합니다. X축과 Y축은 각각 첫 번째와 두 번째 주성분을 나타내며, 각 점은 하나의 이미지 임베딩을 의미합니다. 이 차트를 통해 이미지 간의 유사성과 군집성을 탐색할 수 있으며, 데이터의 잠재적인 구조와 패턴을 파악할 수 있습니다.',
            ...
            'cluster_size_distribution': {
                'title': '클러스터 분포',
                'description': '이 차트는 클러스터링 결과에서 각 클러스터에 속하는 데이터의 개수를 막대 차트 형태로 시각화합니다. X축은 클러스터 번호를, Y축은 해당 클러스터에 속하는 데이터의 개수를 나타내며, 각 막대는 특정 클러스터의 크기를 의미합니다. 이 차트를 통해 클러스터링 결과의 균형과 각 클러스터의 대표성을 평가할 수 있으며, 데이터의 그룹화 패턴을 확인할 수 있습니다.',
                'category': 'clustering_analysis'
            },
            ...
            'cam_distribution': {
                'title': 'CAM 분포',
                'description': '이미지는 원본 이미지 위에 CAM(Class Activation Map)을 히트맵 형태로 시각화합니다. 히트맵은 빨간색(가장 높은 활성화)부터 파란색(가장 낮은 활성화)까지의 색상 스펙트럼을 사용하여, 모델이 특정 예측을 내리는 데 가장 중요하게 고려한 이미지 영역을 시각적으로 강조합니다. 이 오버레이는 모델이 이미지의 어느 부분에 집중하고 있는지를 직관적으로 이해하는 데 도움을 줍니다.<br><br>차트는 CAM 활성화 값의 통계적 분포를 히스토그램 형태로 보여줍니다. X축은 활성화 값을 0.0부터 1.0까지의 범위로 나타내며, Y축은 해당 활성화 값 범위에 속하는 픽셀의 빈도를 표시합니다. 차트 좌측 상단에는 평균, 표준편차, 최대값과 같은 주요 통계와 특정 임계값을 초과하는 활성화 영역의 비율이 요약되어 있습니다. 이 분포는 CAM 값들이 이미지 전체에 걸쳐 어떻게 퍼져 있는지, 즉 모델의 활성화가 특정 영역에 집중되어 있는지 또는 넓게 분포되어 있는지를 파악하는 데 중요한 정보를 제공합니다.',
    ...
    def generate_description_html(self, chart_key):
        ...
        <div class="chart-description">
            <div class="description-header">
                ...
                <span class="description-title">차트 설명</span>
            </div>
