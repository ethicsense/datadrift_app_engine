...
class Config:
    ...
    def __init__(self):
        # Flask 웹앱 디렉토리 기준으로 경로 설정
        self.flask_webapp_dir = Path(__file__).parent.absolute()
        
        # 주요 디렉토리 경로들
        self.db_dir = self.flask_webapp_dir / "db"
        self.models_dir = self.flask_webapp_dir / "models"
        self.datasets_uploads = self.flask_webapp_dir / "datasets" / "uploads"
    ...
    def get_project_paths(self):
        """프로젝트 경로들을 딕셔너리로 반환 (기존 호환성 유지)"""
        return {
            'flask_webapp_dir': str(self.flask_webapp_dir),
