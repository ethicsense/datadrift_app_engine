...
class EmbeddingAnalyzer:
    ...
    def perform_clustering(self, embeddings_data, file_names, file_paths, n_clusters=None, method='kmeans', cluster_selection_method='silhouette'):
        ...
        # 클러스터링은 원본 고차원 임베딩에서 수행하여 의미 정보 손실을 줄임
        clustering_result = self._apply_clustering(embeddings_array, method, n_clusters, cluster_selection_method)
    ...
    def _apply_clustering(self, embeddings_array, method, n_clusters, cluster_selection_method='silhouette'):
        """클러스터링 알고리즘을 적용합니다."""
        if method == 'kmeans':
            if n_clusters is None:
                ...
                if cluster_selection_method == 'silhouette':
                    ...
                    n_clusters = self._find_optimal_clusters_silhouette(embeddings_array)
                elif cluster_selection_method == 'elbow':
                    print("Determining optimal number of clusters using Elbow method...")
                    n_clusters = self._find_optimal_clusters_elbow(embeddings_array)
                elif cluster_selection_method == 'manual':
                    print("Using manual cluster selection...")
                    n_clusters = self._find_optimal_clusters_manual(embeddings_array)
                else:
