...
class ChartDescriptionGenerator:
    ...
    def _initialize_descriptions(self):
        ...
        return {
            ...
            'noise_vs_sharpness': {
                'title': '추정 노이즈 시그마 vs 선명도',
                'description': '이 차트는 Wavelet-MAD 기반 추정 노이즈 시그마와 선명도(라플라시안 분산) 간의 관계를 산점도로 시각화합니다. 가로축은 추정 노이즈 시그마를, 세로축은 선명도 값을 나타내며 각 점은 개별 이미지를 의미합니다. 값의 상호 관계를 통해 노이즈/블러 특성 분포를 확인하고 전처리 우선순위를 파악할 수 있습니다.',
                'category': 'image_attributes'
            },
