...
class ImageAnalysisReport:
    ...
    def create_summary_stats(self):
        ...
        for item in self.attr_data.values():
            ...
            noise_levels.append(
                item.get('estimated_noise_sigma', item.get('gaussian_noise_level', item.get('noise_level', 0.0)))
            )
    ...
    def create_quality_attribute_charts(self):
        ...
        plt.title('Estimated Noise Sigma vs Sharpness', fontsize=14, fontweight='bold')
        plt.xlabel('Estimated Noise Sigma')
    ...
    def create_sample_images_table(self, max_samples=10):
        ...
        for filename, item in samples:
            sample_data.append({
                ...
                'estimated_noise_sigma': f"{item.get('estimated_noise_sigma', item.get('gaussian_noise_level', item.get('noise_level', 0.0))):.4f}",
