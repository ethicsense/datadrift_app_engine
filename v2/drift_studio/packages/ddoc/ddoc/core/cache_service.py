...
class CacheService:
    ...
    def save_analysis_cache(
        ...
        try:
            ...
            # JSON types: summary, attributes*, file_metadata, embedding_index*
            # PKL types: embedding*, xai*
            if (
                cache_type in ["summary", "file_metadata"]
                or cache_type.startswith("attributes_")
                or cache_type.startswith("embedding_index_")
            ):
    ...
    def load_analysis_cache(
        ...
        if (
            ...
            try:
                ...
                # 캐시 파일이 깨진 경우(부분 write/중단 등) → 캐시 미스로 취급
                return None
        elif cache_type.startswith("embedding_") or cache_type.startswith("xai_"):
            cache_file = data_dir / f"{cache_type}.pkl"
            if not cache_file.exists():
