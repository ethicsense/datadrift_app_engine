...
def create_parser():
    """Flask 앱 직접 실행 시 argument parser."""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--flask-port",
        type=int,
        help="Flask / SocketIO listen port (default: from config, usually 5555)",
    )
    parser.add_argument(
        "--fiftyone-port",
        type=int,
        help="FiftyOne helper app port (default: from config)",
    )
...
def get_project_paths():
    """backend 패키지 루트 기준으로 주요 디렉토리 경로들을 반환합니다."""
    try:
        ...
        return {
            ...
            "datasets_uploads": os.path.join(app_root, "datasets", "uploads"),
            "datasets_exported": os.path.join(app_root, "datasets", "exported_datasets"),
            "logs_dir": os.path.join(app_root, "logs"),
            "static_cam_results": os.path.join(app_root, "static", "cam_results"),
            "static_perturbation_results": os.path.join(app_root, "static", "perturbation_results"),
        }
...
if __name__ == "__main__":
    ...
    try:
        ...
        flask_port = args.flask_port if args.flask_port is not None else config.flask_port
        fiftyone_port = (
        ...
        flask_port = args.flask_port if args.flask_port is not None else 5555
        fiftyone_port = args.fiftyone_port if args.fiftyone_port is not None else 8159
        print(
            f"⚠️  Config not available, using fallback values - Flask: {flask_port}, FiftyOne: {fiftyone_port}"
        )

    init_app(app)
    socketio.run(app, port=flask_port, debug=args.debug)