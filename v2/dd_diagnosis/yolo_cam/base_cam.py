...
#                                                    requires_grad=True)
        ...
        # outputs 처리 - YOLO 버전에 따라 다른 형식 처리
        ...
        # output이 여전히 list인 경우 첫 번째 요소 사용
...
#             target_categories = np.argmax(output.cpu().data.numpy(), axis=-1)
            if self.task == 'od':
                # output이 boxes 속성을 가지는지 확인
                if hasattr(output, 'boxes') and hasattr(output.boxes, 'cls'):
                    target_categories = output.boxes.cls
                else:
                    # boxes 속성이 없는 경우 기본값 사용
                    print(f"Warning: output does not have boxes attribute. Type: {type(output)}")
                    target_categories = [0]  # 기본 클래스 ID
            elif self.task == 'cls':
                # Change
                # target_categories = [np.argmax(output.probs.cpu().numpy())]
                if hasattr(output, 'probs') and hasattr(output.probs, 'top5'):
                    target_categories = output.probs.top5
                else:
                    target_categories = [0]  # 기본 클래스 ID
            elif self.task == 'seg':
                if hasattr(output, 'summary'):
                    target_categories = [category['name'] for category in output.summary()]
                else:
                    target_categories = ['default']  # 기본 카테고리
            else:
        ...
        if self.uses_gradients:
            ...
            loss = sum([target(output)
                       for target, output in zip(targets, [output])])
    ...
    def compute_cam_per_layer(
        ...
        # activations와 gradients가 tuple인 경우 처리
